    ...
    Renderer: core::Renderer,
{
    ...
    pub fn new<T>(
        ...
        Self {
            contents,
            id: cosmic::iced_core::widget::Id::unique(),
            width: Length::Fill,
            height: Length::Fill,
            spacing: 0.0,
            on_click: None,
    ...
    pub fn on_dnd_enter(
        mut self,
        dnd_enter_handler: impl Fn(Vec<String>) -> Message + 'static,
    ) -> Self {
    ...
    pub fn on_dnd_leave(mut self, dnd_leave_handler: impl Fn() -> Message + 'static) -> Self {
        self.on_dnd_leave = Some(Box::new(dnd_leave_handler));
        self
    }

    /// set the drag_id
    pub fn drag_id(mut self, drag_id: DragId) -> Self {
...
/// The default style of a [`PaneGrid`].
pub fn cosmic_default(theme: &cosmic::Theme) -> Style {
    Style {
        hovered_region: Highlight {
            background: Background::Color(Color {
                a: 0.5,
