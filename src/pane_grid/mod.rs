...
use cosmic::iced_core::{
    self, layout, touch,
    widget::{tree, Tree},
    Clipboard, Layout, Point, Shell, Size, Widget,
};
use cosmic::iced::{
    clipboard::{
...
use cosmic::iced::core::overlay::{self, Group};
use cosmic::iced::core::renderer;
use cosmic::iced::core::{
    Background, Border, Color, Length, Pixels, Rectangle, Theme, Vector};
...
pub struct PaneGrid<'a, Message, Theme, Renderer>
where
    Theme: Catalog + cosmic::widget::pane_grid::Catalog + 'a,
    ...
    Renderer: cosmic::iced_core::Renderer + 'a,
{
    ...
    pub fn drop_target_from_position(&self, x: f64, y: f64) -> (Pane, DragId)
    {
        ...
        let limits = cosmic::iced::core::layout::Limits::NONE
            .min_width(1.0)
            .min_height(1.0);
        let wsize = Size {width: self.width, height: self.height};
    ...
    pub fn get_contents(&self) -> &Contents<'a, Content<'a, Message, Theme, Renderer>> {
        &self.contents
    }

    pub fn get_contents_mut(&mut self) -> &mut Contents<'a, Content<'a, Message, Theme, Renderer>> {
        &mut self.contents
    }
    ...
    pub fn get_spacing(&self) -> Pixels {
        self.spacing.into()
    }
    ...
    pub fn get_on_drag(&self) -> &Option<Box<dyn Fn(DragEvent) -> Message + 'a>> {
        &self.on_drag
    }

    pub fn get_on_click(&self) -> &Option<Box<dyn Fn(Pane) -> Message + 'a>> {
        &self.on_click
    }

    pub fn get_on_resize(&self) -> &Option<(f32, Box<dyn Fn(ResizeEvent) -> Message + 'a>)> {
        &self.on_resize
    }
    ...
    /// Sets the DragID of the whole PaneGrid
    #[must_use]
...
where
    Theme: crate::pane_grid::Catalog + cosmic::widget::pane_grid::Catalog + 'a,
    Renderer: cosmic::iced_core::Renderer,
    cosmic::widget::PaneGrid<'a, crate::app::Message, Theme, Renderer>:
        std::convert::From<crate::pane_grid::PaneGrid<'a, crate::app::Message, Theme, Renderer>> + 'a,
{
    ...
    fn drag_destinations(
        ...
        let limits = cosmic::iced::core::layout::Limits::NONE
            .min_width(1.0)
            .min_height(1.0);
        let window_size = limits.resolve(
            self.width,
            self.height,
            Size::ZERO,
        );
        for (pane, size) in self
            .contents
            .layout()
            .pane_regions(spacing, window_size)
        {
...
impl<'a, Message, Theme, Renderer>
    From<PaneGrid<'a, Message, Theme, Renderer>>
    for cosmic::widget::PaneGrid<'a, Message, Theme, Renderer>
where
    Theme: Catalog + cosmic::widget::pane_grid::Catalog + 'a,
    Renderer: cosmic::iced_core::Renderer + 'a,
{
    ...
    ) -> cosmic::widget::PaneGrid<'a, Message, Theme, Renderer> {
        pane_grid.into()
    }
}

pub fn layout_region(layout: Layout<'_>, cursor_position: Point) -> Option<Region> {
    let bounds = layout.bounds();
...
pub fn in_edge(layout: Layout<'_>, cursor: Point) -> Option<Edge> {
    let bounds = layout.bounds();
...
pub fn edge_bounds(layout: Layout<'_>, edge: Edge) -> Rectangle {
    let bounds = layout.bounds();
...
pub fn layout_region_bounds(layout: Layout<'_>, region: Region) -> Rectangle {
    let bounds = layout.bounds();
...
impl<'a, T> Contents<'a, T> {
    ...
    pub fn iter_mut(&mut self) -> Box<dyn Iterator<Item = (Pane, &mut T)> + '_> {
        match self {
            Contents::All(contents, _) => {
...
pub type StyleFnTheme<'a, CosmicTheme> = Box<dyn Fn(&CosmicTheme) -> Style + 'a>;

impl Catalog for CosmicTheme {
    type Class<'a> = StyleFnTheme<'a, Self>;

    fn default<'a>() -> StyleFnTheme<'a, Self> {
        Box::new(default_cosmic_theme)
    }

    fn style(&self, class: &StyleFnTheme<'_, Self>) -> Style {
        class(self)
    }
}

/// The default style of a [`PaneGrid`].
pub fn default_cosmic_theme(theme: &CosmicTheme) -> Style {
